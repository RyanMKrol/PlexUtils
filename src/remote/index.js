...
async function fetchRawPlexTelevisionLibraryData() {
  const rawTelevisionLibraryData = await fetchLocalXmlData(`https://192.168.1.130:32400/library/sections/5/all?X-Plex-Token=${process.env.PLEX_API_TOKEN}`);
  ...
  return Promise.all(
    rawTelevisionLibraryData.MediaContainer.Directory
      .map(async (item) => {
        ...
        const seasonsData = await fetchUnspecifiedPlexData(show.childrenUri);
        ...
        const seasons = await Promise.all(dir.map(async (seasonData) => {
          ...
          const episodesDataRaw = await fetchUnspecifiedPlexData(season.childrenUri);
